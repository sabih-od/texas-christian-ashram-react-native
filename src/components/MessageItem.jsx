...
const MessageItem = ({ item, userid, handleDelete }) => {
    ...
    return (
        <View style={[{ marginBottom: 10, maxWidth: '70%', }, sender ? { marginLeft: 'auto', flexDirection: 'row-reverse' } : { marginRight: 'auto', flexDirection: 'row' }]}>
            ...
            <View>
                <Text style={[{ textTransform: 'capitalize', fontFamily: fonts.latoBold, marginBottom: 5, color: colors.black }, sender ? { marginLeft: 'auto' } : { marginRight: 'auto' }]}>{`${item?.user?.first_name} ${item?.user?.last_name}`}</Text>
                {/* {showDelete &&
                    <TouchableOpacity
                        activeOpacity={0.8}
                ...
                <View style={{
                    ...
                    >
                        <Text style={{
                            fontFamily: fonts.latoRegular, fontSize: 14,
...
const styles = StyleSheet.create({
    time: { fontFamily: fonts.latoRegular, fontSize: 12, marginTop: 3 },
    proficon: { width: 40, height: 40, borderRadius: 40, resizeMode: 'cover', }
})
